...
class DayInsuranceProductInfo(models.Model):
    ...
    product_id = fields.Many2one("product.product", string="Rented Product")
    ...
    insurance_percent = fields.Float("Insurance Percent")
...
class InsuranceProductSolInfo(models.Model):
    ...
    sol_id = fields.Many2one("sale.order.line", string="Order Line")
    insurance_product_id = fields.Many2one(
        "product.product",
        string="Product",
        domain=lambda self: [
    ...
    insurance_price_unit = fields.Float(
        "Insurance Amount", compute="_compute_insurance_price_unit"
    )
        ...
        for r in self:
            ...
            if r.insurance_type == "product":
                ...
                r.insurance_price_unit = (
                    r.sol_id.price_unit * r.sol_id.rental_qty * percent / 100
                )
