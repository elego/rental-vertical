...
class RepairOrder(models.Model):
    ...
    date_deadline = fields.Date(
        string="Deadline", help="This date is the planned end date for the repair."
    )

    date_end = fields.Date(
        string="Date Finished", help="This date is the real end date for the repair."
    )
    ...
    def onchange_product_id(self):
        ...
        if self.product_id:
            self.income_analytic_account_id = (
                self.product_id.income_analytic_account_id.id
            )
            self.expense_analytic_account_id = (
                self.product_id.expense_analytic_account_id.id
            )
            ...
            if repair.income_analytic_account_id:
                ...
                for operation in repair.operations:
                    ...
                    if (
                        ...
                        and operation.analytic_tag_ids
                    ):
                        operation.invoice_line_id.analytic_tag_ids = (
                            operation.analytic_tag_ids
                        )
    ...
    def _prepare_expense_analytic_line(self, line):
        ...
        return {
            ...
            "ref": self.name,
            "user_id": self._uid,
            "company_id": self.expense_analytic_account_id.company_id.id
