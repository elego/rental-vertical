...
class ProjectTask(models.Model):
    ...
    def _compute_stagnation_time(self):
        for task in self:
            s = sum([math.ceil((i.end_time - i.start_time).total_seconds()) for i in task.stagnation_ids])
            task.total_stagnation_time_sec = s
            task.total_stagnation_time = self._seconds_to_human_readable(s) if s > 0 else "-"
