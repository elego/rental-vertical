...
class TollChargeLineInvoicing(models.TransientModel):
    ...
    def _onchange_partner_id(self):
        if self.partner_id:
            ...
            self.administrative_charge_product = (
                self.partner_id.administrative_charge_product
            )
    ...
    def action_create_invoice(self):
        """
        Create draft customer invoices for the automatically found partners or
        manually set partner with invoice lines for toll charges and
        administrative charges.
        """
        tcls = self._get_toll_charge_lines()
        if self.automatic_invoicing:
            partners = (
                tcls.filtered(lambda tcl: not tcl.invoiced and tcl.chargeable)
                .mapped("sale_line_id")
                .mapped("order_id")
                .mapped("partner_invoice_id")
            )
        else:
        ...
        for partner in partners:
            ...
            if self.automatic_invoicing:
                partner_tcls = partner_tcls.filtered(
                    lambda tcl: tcl.sale_line_id
                    and tcl.sale_line_id.order_id.partner_invoice_id == partner
                )
            for product in partner_tcls.mapped("product_id"):
                product_tcls = partner_tcls.filtered(
                    lambda tcl: tcl.product_id == product
                )
                iline_values = self._prepare_toll_product_line(
                    invoice, product, product_tcls.filtered("chargeable")
                )
            ...
            if administrative_charge:
                charge_values = self._prepare_administrative_product_line(
                    invoice, charge_product
                )
