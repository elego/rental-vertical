...
class TestRentalTollCollect(TransactionCase):
    ...
    def run_import_toll_charge_lines(self, file_name, file_type):
        with open(os.path.join(_path, "tests/data", file_name), "rb") as file:
            import_wizard = self.env["toll.charge.line.import"].create(
                {"data_file": base64.b64encode(file.read()), "filename": file_name}
            )
            ...
            domain = [
                (
                    "booking_number",
                    "in",
                    ["100000000000051", "100000000000052", "100000000000053"],
                )
            ]
    ...
    def test_02_rental_so_toll_charge_line(self):
        ...
        self.assertEqual(len(self.sale_order.order_line), 1)
        self.assertEqual(self.so_line.product_uom, self.uom_day)
        ...
        self.assertEqual(self.so_line.product_uom_qty, 31.0)
    ...
    def test_03_rental_so_to_invoice_toll_charge_lines(self):
        ...
        self.assertEqual(
            self.invoice.invoice_line_ids.mapped("price_unit"), [200.00, 4.50]
        )
        ...
        self.assertEqual(
            self.invoice.invoice_line_ids.mapped("price_unit"), [200.00, 3.25]
        )
        self.assertEqual(self.sale_order.toll_line_count, 3)
        self.assertEqual(self.sale_order.toll_line_charged_count, 2)

    def test_04_import_csv_toll_charge_lines(self):
        self.run_import_toll_charge_lines("test_toll_charge_line.csv", "text/csv")
