...
from odoo.addons.shipment_plan_sale.tests.test_shipment_plan_sale import (
    confirm_shipment_plan_pos,
)
...
class TestShipmentPlanSale(ShipmentPlanCommon):
    def setUp(self):
        ...
        res = (
            ...
            .with_context(
                active_model="product.product", active_id=self.product_rental.id
            )
            .create(values)
            .create_rental_product()
        )
        self.service_rental = ProductObj.browse(res["res_id"])
        self.service_rental.write({"rental": True, "list_price": 100})
    ...
    def test_00_shipment_plan_rental(self):
        ...
        self.assertEqual(rental_order.order_line, shipment_plan.origin_sale_line_ids)
        self.assertEqual(
            rental_order.order_line, return_shipment_plan.origin_return_sale_line_ids
        )
