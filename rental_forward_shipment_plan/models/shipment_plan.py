...
class ShipmentPlan(models.Model):
    ...
    src_forward_shipment_plan_id = fields.Many2one(
        ...
        string="Source Forward Shipment Plan",
        ondelete="restrict",
        copy=False,
    )
    ...
    dest_forward_shipment_plan_id = fields.Many2one(
        comodel_name="shipment.plan",
        string="Dest Forward Shipment Plan",
        ondelete="restrict",
        copy=False,
    )
    ...
    @api.model
    def _prepare_forward_shipment_plan_values(
        self, src_shipment_plan, dest_shipment_plan
    ):
        res = {
            "name": _("Forward Shipment Plan for [%s - %s]")
            % (src_shipment_plan.name, dest_shipment_plan.name),
            "plan_type": "rental",
