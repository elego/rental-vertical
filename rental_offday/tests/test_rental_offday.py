...
class TestRentalOffDay(TransactionCase):
    ...
    def test_02_daily_rental_offdays(self):
        ...
        self.assertEqual(
            e.exception.name,
            _('The off-day "%s" was already created as fixed off-day.') % date_fixed_offday
        )
        ...
        with self.assertRaises(UserError) as e:
            self.sale_order_line.write({
                'add_offday_ids': [(1, self.sale_order_line.add_offday_ids[0].id, {
        ...
        self.assertEqual(
            e.exception.name,
            _('The off-day "%s" is not between %s and %s.') % (date_before_start, self.date_start, self.date_end)
        )
