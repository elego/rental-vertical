    ...
    def _compute_toll_charged_count(self):
        for rec in self:
            rec.toll_line_charged_count = len(
                rec.contract_line_ids.mapped("sale_order_line_id")
                .mapped("toll_line_ids")
                .filtered("invoiced")
            )
    ...
    def _compute_toll_line_count(self):
        for rec in self:
            rec.toll_line_count = len(
                rec.contract_line_ids.mapped("sale_order_line_id").mapped(
                    "toll_line_ids"
                )
            )
    ...
    def _prepare_invoice_line(self, invoice_id=False, invoice_values=False):
        ...
        if end_date and start_date and self.sale_order_line_id:
            ...
            res.update(
                {
                    "toll_line_ids": [
                        (6, 0, toll_charge_lines.filtered(lambda l: not l.invoiced).ids)
                    ],
                }
            )
        return res
