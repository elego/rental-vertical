        ...
        split_groups: function (events, group_bys) {
            ...
            if (!is_default_product_id) {
                if (group_bys[0] === "product_categ_id") {
                    ...
                    let groups_and_layersGroup = self.generate_sub_groups(
                        groups,
                        group_order_names,
                        "order_name"
                    );
        ...
        generate_sub_groups: function (groups, layer_groups, group_by) {
            ...
            for (let m = 0; m < group_ids.length; m++) {
                const group = groups.find(
                    (elem) =>
                        elem.id ===
                        layer_groups[group_ids[m].layer_group_pos].nestedGroups[
                            group_ids[m].nested_group_pos
                        ]
                );
                ...
                if (group_by === "partner_id") {
                    newGroup.partner_id =
                        layer_groups[group_ids[m].layer_group_pos].partner_id;
                } else if (group_by === "order_name") {
                    newGroup.order_name =
                        layer_groups[group_ids[m].layer_group_pos].order_name;
                }
