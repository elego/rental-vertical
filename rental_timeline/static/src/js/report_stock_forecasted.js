odoo.define("stock.ReplenishReport", function (require) {
    ...
    const ReplenishReport = clientAction.extend({
        ...
        init: function (parent, action, options) {
            ...
            this.actionMethod = `action_product_${
                isTemplate ? "tmpl_" : ""
            }forecast_report`;
            const reportName = `report_product_${
                isTemplate ? "template" : "product"
            }_replenishment`;
        ...
        willStart: function () {
            var loadWarehouses = this._rpc({
                ...
                if (this.context.warehouse) {
                    this.active_warehouse = this.warehouses.find(
                        (w) => w.id == this.context.warehouse
                    );
                } else {
         ...
         */
        start: function () {
            return Promise.all([this._super(...arguments)]).then(() => {
        ...
        async _createGraphController() {
            ...
            const viewRegistryKey =
                viewArchJSClass && viewRegistry.contains(viewArchJSClass)
                    ? viewArchJSClass
                    : "graph";
            ...
            graphController.el.querySelector(
        ...
        _renderButtons: function () {
            ...
            this.$buttons.on(
                "click",
                ".o_report_replenish_buy",
                this._onClickReplenish.bind(this)
            );
        ...
        _bindAdditionalActionHandlers: function () {
            ...
            rr.on(
                "mouseenter",
                ".o_report_replenish_change_priority",
                this._onMouseEnterPriority.bind(this)
            );
            rr.on(
                "mouseleave",
                ".o_report_replenish_change_priority",
                this._onMouseLeavePriority.bind(this)
            );
            rr.on(
                "click",
                ".o_report_replenish_unreserve",
                this._onClickUnreserve.bind(this)
            );
            rr.on(
                "click",
                ".o_report_replenish_reserve",
                this._onClickReserve.bind(this)
            );
        },
