...
class CreateSaleTransRequest(models.TransientModel):
        ...
        if not all_set and not no_set:
            ...
            for line in order_lines:
                ...
                if shipment_plan.id != line.trans_shipment_plan_id.id:
                    raise exceptions.UserError(
                        _('You can not create the purchase request for sale order lines with different Shipment Plan.'))
    ...
    def _prepare_shipment_plan_values(self, order):
        order = self.order_id
        src_address_id = self.env.user.company_id.with_context(
            show_address=True).id
        dest_address_id = order.partner_shipping_id.with_context(
            show_address=True).id
        order_lines = self.origin_line_ids.mapped('order_line_id')
